...
const T = {
  en: {
    ...
    contact_info: [["Direct","mostafa.payandeh@norfox.se"],["Invoices","invoice@norfox.se"],["Location","Stockholm, Sweden"],["Phone","+46 76 284 4244"]],
  ...
  sv: {
    ...
    contact_info: [["Direkt","mostafa.payandeh@norfox.se"],["Faktura","invoice@norfox.se"],["Plats","Stockholm, Sverige"],["Telefon","+46 76 284 4244"]],
...
const CONTACT_ICONS  = ["mail", "file-invoice", "map-pin", "phone"];
...
function PageHeader({ kicker, title, lede, secondaryMark, markOpacity = 0.32 }) {
  const mobile = useMobile();
  return (
    <section style={{ position: "relative", background: "#0c0b16", color: "#FFFFFF", paddingTop: mobile ? 60 : 100, paddingBottom: mobile ? 60 : 100, overflow: "hidden" }}>
      <AuroraBG />
...
// ─── Animated aurora background (homepage hero) ───────────────────────────
function AuroraBG({ embers = true }) {
  const ref = React.useRef(null);
  useEffect(() => {
    const canvas = ref.current;
    if (!canvas || !embers) return;
    const ctx = canvas.getContext("2d");
    let raf, stop = false, particles = [];
    ...
    function spawn(scatter) {
      return {
        ...
        y: scatter ? Math.random() * rect.height : rect.height + 10,
        size: Math.random() * 2.4 + 0.5,
        speedY: Math.random() * 1.6 + 0.5,
        speedX: (Math.random() - 0.5) * 0.6,
        life: Math.random() * 0.5 + 0.5, maxLife: 1,
        hue: Math.random() * 30 + 15,
        brightness: Math.random() * 20 + 55,
    ...
    function loop() {
      ...
      if (W < 1 || H < 1) { raf = requestAnimationFrame(loop); return; }
      ctx.clearRect(0, 0, W, H);
      const target = Math.floor(W * H / 8500);
      while (particles.length < target) { const p = spawn(true); p.maxLife = p.life; particles.push(p); }
      if (Math.random() < 0.4) { const q = spawn(false); q.maxLife = q.life; particles.push(q); }
      particles = particles.filter(p => p.life > 0);
      for (const pt of particles) {
        pt.life -= 0.004;
        pt.speedX = Math.max(-1, Math.min(1, pt.speedX + pt.drift));
        pt.x += pt.speedX; pt.y -= pt.speedY;
        const ratio = Math.max(0, Math.min(1, pt.life / pt.maxLife));
        const alpha = Math.pow(ratio, 0.6) || 0;
        const r = pt.size * (1 + (1 - ratio) * 0.5);
        const g = ctx.createRadialGradient(pt.x, pt.y, 0, pt.x, pt.y, r * 3);
        g.addColorStop(0, "hsla(" + pt.hue + ",100%," + pt.brightness + "%," + alpha + ")");
        g.addColorStop(0.3, "hsla(" + pt.hue + ",100%,45%," + (alpha * 0.6) + ")");
        g.addColorStop(1, "hsla(" + pt.hue + ",80%,30%,0)");
    ...
    loop();
    return () => { stop = true; cancelAnimationFrame(raf); window.removeEventListener("resize", size); };
  }, [embers]);

  const blob = (extra) => ({ position: "absolute", borderRadius: "50%", ...extra });
  return (
    <div aria-hidden="true" style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      <div style={blob({ width: "80vmax", height: "80vmax", left: "-15%", top: "-42%", filter: "blur(90px)", opacity: 0.82, background: "radial-gradient(circle,#2bb8e8 0%,#1f6fd6 42%,transparent 70%)", animation: "aurora-a 22s ease-in-out infinite" })} />
      <div style={blob({ width: "78vmax", height: "78vmax", right: "-18%", top: "-30%", filter: "blur(90px)", opacity: 0.80, background: "radial-gradient(circle,#7b34d6 0%,#4a1f9e 45%,transparent 72%)", animation: "aurora-b 26s ease-in-out infinite" })} />
      <div style={blob({ width: "82vmax", height: "82vmax", right: "-12%", bottom: "-48%", filter: "blur(95px)", opacity: 0.80, background: "radial-gradient(circle,#e83a86 0%,#b3247a 45%,transparent 72%)", animation: "aurora-c 24s ease-in-out infinite" })} />
      <div style={blob({ width: "72vmax", height: "72vmax", left: "-8%", bottom: "-40%", filter: "blur(100px)", opacity: 0.68, background: "radial-gradient(circle,#f0a878 0%,#d6663c 45%,transparent 72%)", animation: "aurora-d 28s ease-in-out infinite reverse" })} />
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 85% 75% at 50% 38%, rgba(8,4,18,0.32) 0%, rgba(8,4,18,0.08) 44%, rgba(8,4,18,0.66) 100%)" }} />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(102deg, rgba(8,7,16,0.80) 0%, rgba(8,7,16,0.46) 36%, rgba(8,7,16,0.06) 70%)" }} />
      {embers && <canvas ref={ref} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block" }} />}
    </div>
  );
}
...
function HomePage() {
  ...
  return (
    <>
      <section style={{ position: "relative", background: "#0c0b16", color: "#FFFFFF", paddingTop: mobile ? 60 : 100, paddingBottom: mobile ? 72 : 120, overflow: "hidden" }}>
        <AuroraBG />
        <div className="container" style={{ position: "relative", zIndex: 1 }}>
        <img src="images/norfox-logo-fox-ladle.png" alt="" aria-hidden="true" loading="lazy"
          ...
          <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: mobile ? "1fr" : "minmax(0, 0.95fr) minmax(0, 1fr)", gap: mobile ? 40 : 80, alignItems: "start" }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: mobile ? "repeat(2, minmax(0, 1fr))" : "1fr 1fr 1fr", gap: 24, borderTop: "1px solid #F05A1A", paddingTop: 28 }}>
      ...
      <section style={{ background: "#FFFFFF", paddingTop: mobile ? 48 : 72, paddingBottom: mobile ? 60 : 100 }}>
        ...
        <div className="container" style={{ marginTop: 80 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(2, minmax(0, 1fr))" : "repeat(3, 1fr)", gap: 16 }}>
            {t.nav_links.map(([l, slug]) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 24 }}>
            {t.sectors.map((s, si) => (
              <div key={s.n} style={{ background: "#FFFFFF", borderLeft: "4px solid #F05A1A", padding: mobile ? "28px 24px" : "36px 32px", borderRadius: 4 }}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: "20px 0 0", display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "8px 16px" }}>
                  {s.needs.map(n => (
    ...
    <>
      ...
      <section style={{ background: "#1F2529", color: "#FFFFFF", paddingTop: 80, paddingBottom: 100 }}>
        <div className="container">
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile?"repeat(2, minmax(0, 1fr))":"repeat(3, 1fr)", gap: 1, background: "#363C44", border: "1px solid #363C44" }}>
...
function ContactPage() {
  ...
  return (
    <>
      ...
      <section style={{ background: "#1F2529", color: "#FFFFFF", paddingTop: mobile ? 56 : 80, paddingBottom: mobile ? 72 : 100 }}>
        <div className="container">
          <SectionLabel color="#F05A1A">{t.contact_direct_kicker}</SectionLabel>
          <div style={{ marginTop: 28, display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(2, minmax(0, 1fr))", gap: 16, maxWidth: 760 }}>
            {t.contact_info.map(([k, v], ci) => {
              const isEmail = v.includes("@");
              const isPhone = /^[\+\d][\d\s\-\(\)\.]+$/.test(v);
              const href = isEmail ? "mailto:" + v : isPhone ? "tel:" + v.replace(/[\s\-\(\)]/g, "") : null;
              const cardStyle = { background: "#262C32", border: "1px solid #363C44", borderLeft: "4px solid #F05A1A", borderRadius: 4, padding: mobile ? "22px 20px" : "26px 26px", color: "inherit", textDecoration: "none", display: "block" };
              const inner = (
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <div style={{ width: 40, height: 40, background: "rgba(240,90,26,0.15)", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Icon name={CONTACT_ICONS[ci]} size={20} color="#F05A1A" />
                    </div>
                    <SectionLabel color="#8A9099">{k}</SectionLabel>
                  </div>
                  <div style={{ fontFamily: "var(--f-display)", fontSize: mobile ? 18 : 22, fontWeight: 700, wordBreak: "break-word", textTransform: isEmail ? "none" : "uppercase" }}>{v}</div>
                </>
              );
              return href
                ? <a key={k} href={href} className="svc-card" style={cardStyle}>{inner}</a>
                : <div key={k} style={cardStyle}>{inner}</div>;
            })}
          </div>
          <a href={LINKEDIN_URL} target="_blank" rel="noopener noreferrer"
            style={{ marginTop: 32, display: "inline-flex", alignItems: "center", gap: 16, background: "#0A66C2", color: "#FFFFFF", borderRadius: 4, padding: mobile ? "16px 24px" : "18px 30px", fontFamily: "var(--f-display)", fontSize: mobile ? 15 : 18, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>
            <img src="images/linked.png" alt="" loading="lazy" style={{ width: 34, height: 34, objectFit: "contain", display: "block" }} />
            {t.linkedin_label} →
          </a>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: mobile?"1fr":"1fr 1fr 1fr", gap: mobile?36:48 }}>
          ...
          <div>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: "16px 0 0", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <li style={{ fontSize: 14 }}><a href="mailto:invoice@norfox.se" style={{ color: "#8A9099" }}>invoice@norfox.se</a></li>
